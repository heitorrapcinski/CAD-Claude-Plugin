...
```markdown
---
title: EV-5-007 · Aprovação exige duas alçadas
aliases: [EV-5-007]
...
# EV-5-007 · Aprovação exige duas alçadas
...
- [[EV-5-007 · Aprovação exige duas alçadas|EV-5-007]] — Aprovação exige duas alçadas
- ⚠️ conflita com [[EV-5-011 · Código implementa uma alçada|EV-5-011]] → ver [[Investigação - Alçadas de Aprovação]]

## SRC-001 · credito/ (Código)
- [[EV-5-011 · Código implementa uma alçada|EV-5-011]] — Código implementa uma alçada
```

## Como preencher

- **Uma nota por evidência.** O id é **`EV-<sessão>-<seq>`** (ver
  [convenções](../knowledge-vault-doc-conventions/SKILL.md)): `<sessão>` do `state.json` e `<seq>`
  sequencial na sessão — ex.: `EV-5-007`. O código vira
...
- **Como esta evidência é referenciada (evite órfãos):** as notas que citam esta evidência
  ...
  `[[EV-5-007 · Aprovação exige duas alçadas|EV-5-007]]`. Um link pelo **código sozinho**
  (`[[EV-5-007]]`) pode não resolver — mesmo com `aliases` — e vira nó órfão no grafo. Como
...
- Mantenha o **MOC Registro de Evidências** atualizado a cada nova `EV`, agrupando por `SRC`
  e domínio/escopo. O Registro consolida a visão do todo — é onde a divergência entre fontes
  fica visível e vira `11 Investigations`.
